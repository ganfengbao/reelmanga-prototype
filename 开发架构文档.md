...
「漫剧工场 REELMANGA」是一款 **AI 一键生成竖屏漫剧** 的 SaaS 平台。用户写/导入剧本，经多 Agent 自动质检定稿、生成视频提示词、生成人物/场景、合成配音，最终产出可发布的漫剧成片，并支持多平台分发与变现。
...
| 维度 | 目标 |
|------|------|
| 商用 | 多租户（按用户隔离）、按量计费、API Key 私有化、内容合规 |
...
| 创作台 | 剧本 AI 写作/导入 → 多 Agent 质检 → 定稿 → 提示词生成 → 提示词校验 → 人物/场景生成 → 分镜台（可编辑/播放） |
...
- **播放器/分镜**：复用原型的 9:16 帧渲染逻辑，素材 URL 从后端 OSS 返回。
...
- 分镜：`shot` 表，含 `scene_id / character_id / camera / emotion / duration / dialogue / prompt / asset_url`。

### 7.5 生图 / 生视频 / 配音任务

- **生图**：`ImageModel.generate()` → 存 OSS → 回写 `shot.asset_url`。
...
可灵/即梦/Runway/Sora 都是「提交 + 轮询/回调」模式，统一封装。**生视频是成本与耗时大头，必须走 MQ + 限流 + 优先级队列**。
...
- 发布：对接抖音/快手/视频号开放平台（各平台授权与上传接口不同，做成 `PublishChannel` 接口）。
...
| `shot` | project_id, seq, scene_id, character_id, camera, emotion, duration, dialogue, prompt, image_url, video_url, audio_url | 分镜 |
...
| `work` | project_id, status, plays, likes, publish_time | 作品/发布 |
...
前端直接还原现有原型的深色设计系统（品牌渐变、9:16 竖屏卡片、玻璃拟态、状态徽标）。将 `styles/` 设计令牌抽成 CSS 变量 + 组件库主题覆盖，保证与 Figma/原型一致。
